package com.tf.transfer.service;
...
import com.tf.transfer.activity.ReceiveFileActivity;
import com.tf.transfer.util.MyClientThread;
import com.tf.transfer.database.SqliteAdapter;
import com.tf.transfer.util.MyClientThread.FileListener;
import com.tf.transfer.util.WifiAdmin;
import com.tf.transfer.util.WifiAdmin.WifiThreadListener;
...
public class ReceiveFileService extends IntentService{
	...
	protected void onHandleIntent(Intent arg0) {
		...
		if(ip.equals("")){
			WifiAdmin.getInstance().connect(wifi_name, new WifiThreadListener() {
				@Override
				public void onEvent(boolean flag) {
					System.out.println("返回了"+flag);
					if(flag){
						final String ip = WifiAdmin.getInstance().getServerIp();
